...
onBeforeMount(async () => {
  data.value = await $trpc.getReadChallenge.query();
});

const save = async () => {
  if (!data.value) return;
  await $trpc.saveReadChallenge.mutate(_cloneDeep(data.value) || {});
};

const load = async () => {
  try {
    read.value = await $trpc.loadFilesFromTag.query('_i_read');
  } catch (e) {

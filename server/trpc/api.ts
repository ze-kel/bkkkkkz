...
export const appRouter = router({
  ...
  loadFilesFromTag: procedure.input(z.string()).query(async ({ input }) => {
    return FileService.loadFilesFromTag(input);
  }),

  loadFileContent: procedure.input(z.string()).query(async ({ input }) => {
    return FileService.getFileContent(input);
  }),

  saveFileContent: procedure.input(zSavedFile).mutation(async ({ input }) => {
    const currentTime = new Date();
    FileUpdates.ignored[input.path] = new Date(currentTime.getTime() + 3000);
    return await FileService.saveFileContent(input);
  }),

  getOpenedTabs: procedure.query(async () => {
    return await getOpenedTabs();
  }),

  setOpenedTabs: procedure.input(ZOpenedTabs).mutation(async ({ input }) => {
  ...
  getRootPath: procedure.query(() => {
    return getRootPath();
  }),

  setRootPath: procedure.mutation(async () => {
  ...
  saveSettings: procedure.input(zSettings).mutation(async ({ input }) => {
    return await saveSettings(input);
  }),

  getReadChallenge: procedure.query(async () => {
    return await getReadChallenge();
  }),

  saveReadChallenge: procedure.input(zReadChallenge).mutation(async ({ input }) => {
    return await saveReadChallenge(input);
  }),

  isTest: procedure.query(() => {
    return process.env['TEST_MODE'];
  }),

  fileUpdate: procedure.subscription(() => {
  ...
  fileRemove: procedure.subscription(() => {
  ...
  fileAdd: procedure.subscription(() => {
  ...
  treeUpdate: procedure.subscription(() => {
  ...
  tagsUpdate: procedure.subscription(() => {
  ...
  sendNotification: procedure.subscription(() => {
  ...
  clearAllEvents: procedure.mutation(() => {
